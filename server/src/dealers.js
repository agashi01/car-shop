const create = (db) => (req, res) => {
    ...
    db('dealers')
        .insert({ date_of_creation: new Date(), date_of_last_update: new Date(), dealer_name: name }).returning('*')
        .then(dealer => {
            db('dealers').where({ id: dealer[0].id }).select('*').then(result => res.json({ dealer: result[0] }))
        })
        .catch(err => {
            console.log(err)
...
const readAll = (db) => (req, res) => {
    db.select('*').from('dealers').returning('*')
        .then(result => {
            res.json(result)
        })
        .catch(err => {
