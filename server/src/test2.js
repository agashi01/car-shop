...
const runOnce = (db) => (req, res) => {
    ...
    const createCar = async(brand, brandModel, mileage, color, transmission, fuel_type, vehicle_type, dealer_id) => {
                ...
                for (let d = 0; d < transmission.length; d++) {
                    for (let e = 0; e < fuel_type.length; e++) {
                        for (let f = 0; f < vehicle_type.length; f++) {
                            createCar(brand[a], brandModel[brand[a]][j], mileage(), color[c], transmission[d], fuel_type[e], vehicle_type[f], dealer_id[dealer()])
                        }
                    }
